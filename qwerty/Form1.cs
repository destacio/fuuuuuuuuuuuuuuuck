...
namespace qwerty
{
    public partial class Form1 : Form
    {
        ...
        combatMap cMap = new combatMap(10, 5);  // создаем поле боя с указанной размерностью
        ...
        public void Draw()//рисование карты без выделений
        {
            ...
            for (int i = 0; i < cMap.boxes.Count; i++)
            {
                ...
                // ВНИМАНИЕ!!!!
                // ВСЕ Подсвечиваемые клетки определяются ТУПЫМ ИНДУССКИМ АБЫ КАК РАБОТАЮЩИМ,
                // старым алгоритмом, основанном на вычислении айдишника клетки!!! 
                // ..будет переделано
            ...
            for (int i = 0; i < cMap.boxes.Count; i++)
            {
                ...
                else if (activeShip != null)
                {
                    ...
                    if (cMap.boxes[select].spaceObject == null)
                    {
                        ...
                        && cMap.boxes[select].spaceObject.objectType == Constants.SHIP)
                        ...
                        else if (cMap.boxes[select].spaceObject.player != activePlayer)
                    {
                        ...
                        switch (activeShip.attackRange)
                        {
                            case 1:
                                // перемещение на одну клетку вверх
                                if (cMap.boxes[a].y != 0
                                    && cMap.boxes[a].x == cMap.boxes[select].x
                                    && cMap.boxes[a].y - 2 == cMap.boxes[select].y)
                                {
                                    flag = 1;
                                }
                                // перемещение на одну клетку вниз
                                else if (cMap.boxes[a].y + 1 != cMap.height * 2
                                    && cMap.boxes[a].x == cMap.boxes[select].x
                                    && cMap.boxes[a].y + 2 == cMap.boxes[select].y)
                                {
                                    flag = 1;
                        ...
                        if (flag == 1)
                        {
                            activeShip.attack(ref cMap, select);
                            if (activeShip.actionsLeft == 0) activeShip = null;
                            flag = 0;
                            Draw();
                            break;
